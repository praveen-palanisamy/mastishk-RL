...
class Shallow_Q_Learner(object):
	def __init__(self, state_shape, action_shape, neurons=10, learning_rate=0.005, gamma=0.98):
		...
		self.Q = SLP(state_shape, action_shape, self.neurons, device=torch.device(device)).to(device)
	...
	def epsilon_greedy_Q(self, observation):
		...
		if random.random() < self.greedy_threshold:
			...
			action = np.argmax(self.Q(observation).data.to(torch.device('cpu')).numpy())   #Exploitation
